...
import type { DoutorForm } from '@/interfaces/doutor'
...
const submitForm = async () => {
  try {
    ...
    const response = await request<DoutorForm, null>({
...
const loadForm = async () => {
  ...
  if (pageMode === PageMode.PAGE_UPDATE) {
    const doctorFormRequest = request<undefined, DoutorForm>({
      method: 'GET',
